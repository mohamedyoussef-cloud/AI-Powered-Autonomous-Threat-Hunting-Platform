# app.py
# Task 65 + 66: Backend API with Real Components
# ================================================
...
from integration import run_full_pipeline
...
# ============================================
# Section 4: Hunt Pipeline (Background)
# ============================================

def run_hunt_pipeline(hunt_id: str, hypothesis: str, technique_id: str):
    conn = get_db()
    try:
        ...
        if result["status"] == "failed":
            conn.execute(
                ...
                ("failed", result.get("error", "Unknown error"), hunt_id)
            )
            conn.commit()
            return
        ...
        conn.execute(
            ...
            (finding["sigma_rule"], finding["spl_query"], hunt_id)
        )
        conn.commit()
        ...
        conn.execute(
            ...
            (
                ...
                finding["event_count"],
                json.dumps(finding["events"]),
                finding["hunt_timestamp"],
                finding["triage_label"],
                finding["triage_label_name"],
                finding["triage_confidence"],
            )
        )
...
def get_findings(hunt_id: str):
    ...
    try:
        rows = conn.execute(
            "SELECT * FROM findings WHERE hunt_id=?", (hunt_id,)
        ).fetchall()
        findings = []
        for row in rows:
            f = dict(row)
    ...
    try:
        ...
        conn.execute(
            """INSERT INTO verdicts
               ...
               VALUES (?,?,?,?,?,?,?)""",
            (verdict_id, finding_id, row["hunt_id"],
             req.verdict, req.notes, req.analyst, now)
        )
        conn.execute(
