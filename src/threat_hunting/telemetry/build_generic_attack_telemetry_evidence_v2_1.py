...
SOURCE_POLICY_FILE = (
    ...
    / "config"
    / "telemetry"
    / "source_identity_policy_v2_1.json"
)
...
COMPONENT_CSV = (
    OUTPUT_DIR
    / "attack_data_component_evidence_v2_1.csv"
)
...
TECHNIQUE_CSV = (
    OUTPUT_DIR
    / "technique_telemetry_evidence_v2_1.csv"
)

REPORT_OUTPUT = (
    REPORT_DIR
    / "generic_attack_telemetry_evidence_v2_1_summary.json"
)
...
def source_match_score(
    ...
    # Canonical alias equality.
    #
    # Example:
    # WinEventLog -> windowseventlog
    # EVTX        -> windowseventlog
    #
    # This is explicit semantic normalization,
    # not fuzzy substring matching.
    if req_tokens == cand_tokens:
        return (
            0.98,
            "canonical_alias_exact",
        )
    ...
    # -------------------------------------------------
    # Generic single-token containment is NOT enough.
    #
    ...
    #
    # If the ATT&CK requirement itself genuinely is
    # exactly "Process", then identity_exact above
    # still permits "Process" -> "process".
    # -------------------------------------------------

    if (
        len(req_tokens) == 1
        or len(cand_tokens) == 1
    ):
        return (
            0.0,
            "rejected_single_token_overlap",
        )

    # A multi-token source identity needs at least
    # two semantic tokens in common.
    if len(overlap) < 2:
        return (
            0.0,
            "rejected_weak_overlap",
        )
    ...
    # Explicit alias-family overlap strengthens
    # the match but does not replace the requirement
    # for multi-token semantic agreement.
    if (
        overlap
        & alias_group_names
    ):
